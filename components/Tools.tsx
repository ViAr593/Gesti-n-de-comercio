...
import { Product, Supplier, Customer, Employee, Sale, Expense, Quotation, InventoryLog } from '../types';
...
export const Tools: React.FC<ToolsProps> = ({ 
  ...
  const generateSQL = () => {
    let sql = `-- MySQL Database Schema & Data Export generated by GestorPro\n-- Date: ${new Date().toISOString()}\n\n`;
    ...
    if(suppliers.length) {
        sql += `INSERT INTO suppliers (id, name, contact_name, phone, email) VALUES \n`;
        sql += suppliers.map(s => `('${s.id}', '${s.name.replace(/'/g, "''")}', '${s.contactName.replace(/'/g, "''")}', '${s.phone}', '${s.email}')`).join(',\n') + ';\n\n';
    }
    ...
    if(customers.length) {
        sql += `INSERT INTO customers (id, name, tax_id, email, phone, address) VALUES \n`;
        sql += customers.map(c => `('${c.id}', '${c.name.replace(/'/g, "''")}', '${c.taxId}', '${c.email}', '${c.phone}', '${c.address.replace(/'/g, "''")}')`).join(',\n') + ';\n\n';
    }
    ...
    if(employees.length) {
        sql += `INSERT INTO employees (id, name, role, phone, email, password_hash) VALUES \n`;
        sql += employees.map(e => `('${e.id}', '${e.name.replace(/'/g, "''")}', '${e.role}', '${e.phone}', '${e.email}', '${e.password}')`).join(',\n') + ';\n\n';
    }
    ...
    if(products.length) {
        sql += `INSERT INTO products (id, name, description, price, cost, stock, min_stock, category, supplier_id, measurement_unit, measurement_value) VALUES \n`;
        sql += products.map(p => `('${p.id}', '${p.name.replace(/'/g, "''")}', '${p.description.replace(/'/g, "''")}', ${p.price}, ${p.cost}, ${p.stock}, ${p.minStock}, '${p.category}', '${p.supplierId}', '${p.measurementUnit}', ${p.measurementValue})`).join(',\n') + ';\n\n';
    }
    ...
    if(sales.length) {
        sql += `INSERT INTO sales (id, date, total, payment_method, customer_name, customer_id) VALUES \n`;
        sql += sales.map(s => `('${s.id}', '${s.date.slice(0, 19).replace('T', ' ')}', ${s.total}, '${s.paymentMethod}', '${(s.customerName || '').replace(/'/g, "''")}', ${s.customerId ? `'${s.customerId}'` : 'NULL'})`).join(',\n') + ';\n\n';
        
        const allItems = sales.flatMap(s => s.items.map(i => ({...i, saleId: s.id})));
        if(allItems.length > 0) {
            sql += `INSERT INTO sale_items (sale_id, product_id, product_name, quantity, price, discount) VALUES \n`;
            sql += allItems.map(i => `('${i.saleId}', '${i.id}', '${i.name.replace(/'/g, "''")}', ${i.quantity}, ${i.price}, ${i.discount || 0})`).join(',\n') + ';\n\n';
        }
    }
    ...
    if(expenses.length) {
        sql += `INSERT INTO expenses (id, date, description, amount, category) VALUES \n`;
        sql += expenses.map(e => `('${e.id}', '${e.date.slice(0, 19).replace('T', ' ')}', '${e.description.replace(/'/g, "''")}', ${e.amount}, '${e.category}')`).join(',\n') + ';\n\n';
    }
    
    // 7. Inventory Logs
    sql += `-- TABLE: inventory_logs\n`;
    ...
    if(inventoryLogs.length) {
        sql += `INSERT INTO inventory_logs (id, date, product_id, product_name, type, quantity, user_id, user_name) VALUES \n`;
        sql += inventoryLogs.map(l => `('${l.id}', '${l.date.slice(0, 19).replace('T', ' ')}', '${l.productId}', '${l.productName.replace(/'/g, "''")}', '${l.type}', ${l.quantity}, '${l.userId}', '${l.userName.replace(/'/g, "''")}')`).join(',\n') + ';\n\n';
    }
